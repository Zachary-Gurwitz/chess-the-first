...
public class Queen extends Piece{
	...
	public boolean validDirection(String start, String finish) {
		if(Board.squaresMap.containsKey(start) && Board.squaresMap.containsKey(finish)) {
			if(sameDiagonal(start, finish)||((sameRankMove(start, finish) || sameFileMove(start, finish)) && !(sameRankMove(start, finish) && sameFileMove(start, finish)))) {
				return true;
			}
		}
		return false;
